...
~Conjugar~ starts on the ~Browse~ tab. This tab lists thousands of Spanish verbs with their English translations. The most common verbs also show their frequency ranks; "ser", the most common verb of all, is #1. Sort the verbs by frequency or alphabetically by tapping the yellow control above the tab bar. When you find a verb you would like to know more about, tap it. ~Conjugar~ then displays a screen with ~all~ conjugations of that verb. This screen is helpful for learning because it calls out, by highlighting in $RED$, parts of conjugations that are irregular. For example, the %presente de indicativo%, first-person singular conjugation of "dar" has an irregular "y", so ~Conjugar~ displays this conjugation as "$doY$". Tap any conjugation to hear it spoken. Tap ~Browse~ in the top-left corner to return to the ~Browse~ tab.
