...
// they share the stem and differ only in the endings. Because `RegularRoot2`'s
...
// `StemFeature2.contractedFuture` that swaps the stem (hac→ha, dec→di): ha+ré =
// haré, di+ré = diré.
struct FutureEndings2: Feature2 {
  ...
  static let fDrope = FutureEndings2(connector: "")  // haber→habr-, saber→sabr-, querer→querr-
  static let fDr = FutureEndings2(connector: "d")    // tener→tendr-, poner→pondr-, salir→saldr-
  static let fContract = FutureEndings2(connector: "") // hacer→har-, decir→dir- (+ contractedFuture stem)
}
