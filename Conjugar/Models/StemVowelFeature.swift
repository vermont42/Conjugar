...
// Stem-vowel diphthongs (in STR) and -ir weak-slot raising (in WK) are,
// mechanically, the **same operation**: replace the last `from` vowel of the
...
// slots (WK, sentir/dormir-style raising). So one parameterized feature covers
// both; the catalog below is just its instances.
//
// This is end-anchored — it rewrites the **last** occurrence of the trigger
// vowel — so it rides free on prefixed verbs (`comprobar` → compruebo,
// `repetir` → repito) and composes through the existing `Conjugator.compose` seam.
//
// **The STR/WK split.** In an -ir verb that both
...
nonisolated struct StemVowel: ConjugationFeature {
  /// Which named slot set this stem-vowel change fires in.
  enum Slots {
    case str // diphthongs and pedir-style raise (`r-ei-str`)
    case wk  // -ir weak-slot raising (`r-ei-wk`, `r-ou-wk`)
