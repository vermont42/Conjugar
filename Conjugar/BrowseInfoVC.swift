...
class BrowseInfoVC: UIViewController, UITableViewDelegate, UITableViewDataSource, InfoDelegate {
  ...
  override func viewWillAppear(_ animated: Bool) {
    ...
    GlobalContainer.analytics.recordVisitation(viewController: "\(BrowseInfoVC.self)")
  }

  private func updateDifficultyControl() {
    switch GlobalContainer.settings.infoDifficulty {
    case .easy:
  ...
  @objc func difficultyChanged(_ sender: UISegmentedControl) {
    ...
    if index == 0 {
      GlobalContainer.settings.infoDifficulty = .easy
    } else if index == 1 {
      GlobalContainer.settings.infoDifficulty = .moderate
    } else /* index == 2 */ {
      GlobalContainer.settings.infoDifficulty = .difficult
    }
