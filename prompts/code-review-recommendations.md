...
*(Done: `MainTabView` moved to `Tab` builders keyed on `AppTab`. The iconography was unified into one **outline** family rather than mixing fills: Info → stock `info.circle`, Settings → stock `gearshape`, Models `key.fill` → outline `key`, and the two glyphs with real character — the flamenco dancer (Browse) and bull (Quiz) — were kept as **custom SF Symbols** rather than dropped. Two findings worth recording. First, iOS tab bars auto-substitute the `.fill` variant of any SF Symbol; the `TabView`-level `\.symbolVariants` environment does **not** reach the tab-bar chrome, so the three stock-symbol tabs use the explicit-label `Tab(value:content:label:)` form with `.symbolVariants(.none)` pinned on each `Label` to hold the outline family. Second, the custom dancer/bull were built without a vector editor: two royalty-free Noun Project line icons (bull by taash5studio, flamenco dance by Amethyst Studio, both CC BY 3.0) were spliced by script into a `gearshape` template exported from SF Symbols.app — fit to each weight/scale slot's reference bbox, and weight-matched to `key`/`gearshape` by geometric dilation of the fill (the symbol compiler honours `fill`, not `stroke`; `<defs>`/`<use>` keeps each symbol ~90 KB). Custom symbols take `image:`, not `systemImage:`, and aren't auto-filled. Old `Browse`/`Quiz`/`Info`/`Settings`/`Dancer` bitmaps deleted; Info-tab credits updated to the new artists (CC BY 3.0) and the stale gear/question-mark credit removed. Verified in light and dark. Build green, SwiftLint clean.)*
...
*(Done: both browse lists now use `NavigationLink(value:)` (Browse Verbs → its `String` destination, Models → its `ModelInfo` destination) and `ModelView`'s "verbs using this model" rows use a `Button` wrapper around the existing closure navigation — all with `.buttonStyle(.plain)`, so every row gets `.isButton` semantics + a press highlight; the dead `VerbRow` helper was deleted. `ConjugationText.attributedString(for:)` tags the whole form `languageIdentifier = "es"` (needing an explicit `import Foundation` under `MEMBER_IMPORT_VISIBILITY`), so VoiceOver pronounces the Spanish forms with Spanish rules across the Verb, Model, and Results screens — restoring what the retired `setAccessibilityLabelInSpanish` did. `speakOnTapFlash` now also exposes a named `.accessibilityAction(named: L.Accessibility.speak)` (new `Accessibility.speak` catalog key, "Speak"/"Pronunciar") so VoiceOver users can trigger the same pronounce-aloud affordance the plain tap gesture skips for them. Build green, 412 tests / 0 failures, SwiftLint clean.)*
...
*(Done: the `VerbFamilies`↔`VerbMap` guard test and the `GameCenter`-flow tests both landed earlier — `VerbFamiliesTests` in Phase 1, and `GameCenterPromptTests` + the straightened, idempotent `GameCenterFakeTests` in Phase 3. This phase adds the golden-form pins: a new `QuizGoldenFormsTests` suite drives 58 (verb, tense, person) triples — verbs drawn from **every** `VerbFamilies` list, paired with the tense that list is quizzed at — through `TenseBridge`, comparing the **lowercased** (marking-stripped) output to independently-hand-written Spanish, so it pins linguistic truth, not the engine's own output; all 58 matched on the first run, confirming both the forms and the engine. `SettingsViewTests` drops the vacuous `body is (any View)` for a real crash smoke test (`_ = settingsView.body`). `QuizTests` now iterates **both** `shouldShuffle` values, so the deterministic (`false`) run pins reproducible content while the `true` run still exercises the shuffle path. Build green, 412 tests / 0 failures, SwiftLint clean.)*
...
- ◐ **Style:** the diacritic identifiers (`viewContröller`, `scöre`, `identifīer`, `delegāte`, the flag-emoji constant in `AnalyticsLocale.swift:24`) are charming but hostile to search and code review — consider retiring them as files get touched (the `AnalyticsService` protocol's string-constant *requirements* can become one private enum when TelemetryDeck lands). *(Framed as opportunistic "as files get touched" — retired `expectatiön` in the rewritten `RatingsFetcherTests`; the rest live in files this phase didn't need to open, and the `AnalyticsService` constant→enum is explicitly deferred to the TelemetryDeck work, so they're left in place.)*

*(Done: the junk `""` catalog key is deleted and the four `Picker("")` + two `TextField("")` unlabeled controls use explicit `Text(verbatim: "")` labels (plus an `.accessibilityLabel` on the quiz answer field) so extraction can't regenerate it. `RatingsFetcher` is now `ratingsDescription() async -> String?` — `URLSession.data(for:)` + a `Decodable` `LookupResponse` replace the completion handler + `JSONSerialization` dictionary spelunking — and `SettingsView` calls it from a `.task`, showing a new `Settings.ratingsUnavailable` message on failure instead of a silently-empty row; the Spanish exhortation moved to a `Settings.beFirst` catalog key (kept Spanish in both localizations by design). `AppRouter` uses `url.host()`; the widget target's Debug+Release configs gain `MEMBER_IMPORT_VISIBILITY` (which surfaced three latent missing `import WidgetKit`s in the widget views — now fixed); the unused `aps-environment` entitlement is dropped (the commun fetch is poll-at-launch, nothing registers for push). Explanatory comments now pin the persisted-raw-value contract on both `SecondSingular*` enums and the deliberate mixed-gender `él`/`ellas` pronoun pairing. `RatingsFetcherTests` is adapted to the async API (kept as XCTest on purpose — its global-`Current` mutation would race a parallel Swift Testing suite). Build green, 412 tests / 0 failures, SwiftLint clean.)*
...
5. ✅ **View-layer hygiene** *(items 3, 13, 14)* — browse filtering into `onChange` state + sound out of `body`; appearance config consolidated/retired after on-simulator verification; snapshot refresh off-main + date-gated; `ModelView` grid cached in `init`. *(Done: `BrowseSearch` is now a pure `nonisolated` filter; both browse screens materialize the filtered list into `@State`, recomputed only on `searchText`/`sort` change — halving per-keystroke work and moving the no-results sad trombone to the one-shot `.onChange` transition, out of `body`. On-simulator verification showed the pre-iOS-13 tab-/nav-bar `barTintColor`/`tintColor`/`titleTextAttributes` config was fully inert on iOS 26 Liquid Glass (selected tab stayed system-blue, large titles + back chevron stayed default), so both bar-config methods were deleted; the one appearance that does matter — yellow segmented-control titles — moved from the per-`body` `SettingsView.init` to a once-at-launch `AppDelegate` call, with before/after screenshots pixel-identical. `WidgetSnapshotWriter.refresh()` now runs off-main via `Task.detached` at both call sites and is date-gated (skips the rewrite + `reloadAllTimelines()` when today's snapshot is already on disk); `ModelView` precomputes its 6×6 grid conjugations in `init` instead of re-conjugating 36 slots per `body`. Build green, 411 tests / 0 failures, SwiftLint clean.)*
6. ✅ **Quiz + services internals** *(items 12, 15, 16, 6)* — `Cycler`, non-optional dependencies, single shuffle, graceful `process` failure; tutor poll scoped to visibility + per-instance tool counter; single audio-session owner with an explicit category decision. *(Done: `Quiz`'s 13 hand-copied cycling accessors + parallel index vars collapse into a `Cycler` cursor type and a single `allCyclers` restart loop; `settings`/`gameCenter` are now non-optional `let` (three `fatalError` guards deleted); the double `questions.shuffled().shuffled()` is one `shuffle()`; `process`'s `default: fatalError()` degrades to a logged miss + advance instead of crashing a learner. The tutor's infinite 5-s availability poll is gone — `LanguageModelServiceReal` now exposes `start/stopAvailabilityMonitoring()`, driven by `InfoBrowseView.onAppear`/`onDisappear` and self-terminating once the model reports available. `ConjugationTool`'s `nonisolated(unsafe)` static counter becomes a per-instance `OSAllocatedUnfairLock`, reset through the one tool instance the service reuses across sessions — race-free and no longer globally shared with `TutorTestView`. `Utterer` is now the single `AVAudioSession` owner, configured once at launch to `.ambient` (mix + respect the silent switch); `SoundPlayer` no longer sets a competing category, its `print`s become `Logger`, and its `Int.random(in:)` picks become `randomElement()`; `Utterer.settings` is a non-optional injected default. Build green, 411 tests / 0 failures, SwiftLint clean. The 2016-era `SoundPlayer.play(.silence)` speech workaround was retained pending a physical-device check.)*
7. ✅ **Modernization + accessibility + polish** *(items 17, 18, 19-remainder, 20)* — `Tab` API + unified icons, row semantics + Spanish speech attributes, golden-form tests + deterministic `QuizTests`, and the micro grab-bag. *(Item 17 done earlier: `Tab` API + one outline icon family — stock `key`/`info.circle`/`gearshape` held to outline via per-`Label` `symbolVariants(.none)`, plus custom `dancer`/`bull` symbols script-spliced from CC BY 3.0 Noun Project line art into a `gearshape` template and weight-matched by fill dilation; bitmaps deleted, credits updated, verified light + dark. Item 18: browse/detail rows became real `NavigationLink`/`Button`s (button semantics + press highlight) and conjugation `Text` carries `languageIdentifier = "es"` so VoiceOver pronounces forms in Spanish, plus a named `speak` accessibility action on tap-to-hear targets. Item 19-remainder: a 58-case `QuizGoldenFormsTests` pins quiz-list verbs to hand-written Spanish, the tautological `SettingsViewTests` assertion became a real crash smoke test, and `QuizTests` iterates both shuffle modes (the guard test and Game Center tests landed in Phases 1 & 3). Item 20: junk `""` catalog key removed, `RatingsFetcher` → `async`/`await` + `Codable` with a surfaced error and the exhortation in the catalog, `url.host()`, widget `MEMBER_IMPORT_VISIBILITY` (surfacing three latent missing imports), dropped unused `aps-environment`, and clarifying comments. Build green, 412 tests / 0 failures, SwiftLint clean.)*
