...
7. **Modernization + accessibility + polish** *(items 17, 18, 19-remainder, 20)* — `Tab` API + unified icons, row semantics + Spanish speech attributes, golden-form tests + deterministic `QuizTests`, and the micro grab-bag. *(Item 17 done: `Tab` API + one outline icon family — stock `key`/`info.circle`/`gearshape` held to outline via per-`Label` `symbolVariants(.none)`, plus custom `dancer`/`bull` symbols script-spliced from CC BY 3.0 Noun Project line art into a `gearshape` template and weight-matched by fill dilation; bitmaps deleted, credits updated, verified light + dark. Items 18, 19-remainder, and 20 still open.)*
